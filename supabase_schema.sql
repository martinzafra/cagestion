...
CREATE TYPE expense_type_enum AS ENUM ('INVOICE', 'PAYMENT', 'PLATFORM INV.', 'OWNERS EXPENSE');
...
CREATE TABLE IF NOT EXISTS inventory_apartments (
  ...
  settle_cleaning_charge BOOLEAN NOT NULL DEFAULT false, -- pass the cleaning charge/cost through on the statement instead of absorbing it as CA margin
...
-- The Bookings calendar shows every apartment's bookings to every registered
-- user regardless of agent assignment (list view/To Do/Revenue/Expenses keep
-- the can_access_apartment() scoping above). SECURITY DEFINER bypasses RLS
